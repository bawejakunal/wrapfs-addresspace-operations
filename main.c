...
/* Global variable to store mount options.
 */
struct wrapfs_options wopt;
...
static int wrapfs_read_super(struct super_block *sb, void *raw_data, int silent)
{
	...
	((struct wrapfs_sb_info *)(sb)->s_fs_info)->mount_options = wopt;
			    ...
			    const char *dev_name, void *raw_data)
{
	void *lower_path_name = (void *) dev_name;
        ...
        if (!wrapfs_parse_options(raw_data, &wopt, false)) {
                printk(KERN_ALERT "Failed in parsing options\n");
        }
        /*if (wopt.mmap == 1)
                printk(KERN_ALERT "mmap option specified\n");
        else
                printk(KERN_ALERT "mmap option not specified\n");
		*/
